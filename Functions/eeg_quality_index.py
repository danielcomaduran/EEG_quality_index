...
class EEGQualityIndex:
    def __init__(
        ...
        """
            ...
            [Flicking et al. 2019](https://ieeexplore.ieee.org/abstract/document/8936246?msclkid=e2428da6d08b11ecaf6e4f0d1e320c34).

            Parameters
            ----------
                clean_eeg: array_like
                    2D EEG array with no artifacts [channels, samples]
                test_eeg: array_like
                    2D EEG array with artifacts [channels, samples]
                srate_clean: float
                    Sample rate of the clean EEG [Hz]
                srate_test: float (optional)
                    Sample rate of the test EEG [Hz]
                    If None, it will be assumed that the sample rate of the test EEG is equal to the clean EEG
        ...
        self.srate_clean = srate_clean
        self.srate_test = srate_test if srate_test is not None else srate_clean
    ...
    @property
    def clean_eeg(self):
        return self._clean_eeg

    @clean_eeg.setter
    def clean_eeg(self, value):
        self._clean_eeg = self._validate_eeg(value, "clean_eeg")

    @property
    def test_eeg(self):
        return self._test_eeg

    @test_eeg.setter
    def test_eeg(self, value):
        self._test_eeg = self._validate_eeg(value, "test_eeg")
    ...
    def window(self, value):
        self._window = self._validate_window_slide(value, "window")
    ...
    def scoring(self) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        if self.srate_clean != self.srate_test:
            # Run EEG Quality Index on clean and test data
            eqi_clean = self.eqi_zscore(self.clean_eeg, self.srate_clean, sliding=self.sliding, window=self.window[0], slide=self.slide[0])
            eqi_test = self.eqi_zscore(self.test_eeg, self.srate_test, sliding=self.sliding, window=self.window[1], slide=self.slide[1])
        else:
            # Run EEG Quality Index on clean and test data
            eqi_clean = self.eqi_zscore(self.clean_eeg, self.srate_clean, sliding=self.sliding, window=self.window, slide=self.slide)
            eqi_test = self.eqi_zscore(self.test_eeg, self.srate_test, sliding=self.sliding, window=self.window, slide=self.slide)
        ...
        # EQI average
        self.eqi_mean = np.mean(z_scores, 2)                                             # EQI Averages
        self.clean_percent = 100 - np.nanmean((z_scores>0).astype(int), 2)*100           # Percentage of windows > 0
        self.clean_total = 100 - np.nanmean((np.sum(z_scores,0)>0).astype(int), 1)*100   # Percentage of summed windows > 0
        
        return self.clean_total, self.clean_percent, self.eqi_mean
    ...
    def eqi_zscore(
        ...
        ## Zero-crossing rate
        eeg_zcr = self._zero_crossing_rate(eeg_windowed)
    ...
    def _sliding_window(self, data, window, slide):
        """
    ...
    def _validate_eeg(self, value, name):
    ...
    def _validate_window_slide(self, value, name):
            if self.sliding:
                if value is None:
                    raise ValueError(f"{name} must be provided if sliding is True")
                if isinstance(value, list):
                    if len(value) != 2:
                        raise ValueError(f"{name} must be a list of two integers if clean and test have different sample rates")
                    if not all(isinstance(v, int) for v in value):
                        raise ValueError(f"{name} list must contain integers")
                elif not isinstance(value, int):
                    raise ValueError(f"{name} must be an integer or a list of two integers")
            return value